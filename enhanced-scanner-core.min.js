...
(function () {
    ...
    window.EnhancedScanner = {
        config: {
            ...
            reportWorkerBaseUrl: window.selmaDebugLocal
                ? "http://localhost:8787"
                : "https://reports.selma.se",
            reportWorkerInternalSecret: "0d40c67ae1d3b3ac1711dfa7ec912438d2f066cefde9b3a662c971ab77839234",
        },
        ...
        handleReportContactFormSubmit: function (e, form) {
            ...
            if (!this.isValidSwedishPhone(phoneRaw)) {
                console.log("[Scanner] Swedish phone validation failed", { phoneRaw });
                if (phoneErr) {
                    phoneErr.textContent = "Vänligen ange ett giltigt svenskt telefonnummer (t.ex. 070-555 12 34 eller +46 70 555 12 34).";
                    phoneErr.style.display = "block";
                }
                if (phoneField) phoneField.focus();
                return;
            }
            console.log("[Scanner] Swedish phone validation OK");
            if (phoneErr) {
        ...
        /** Swedish numbers: E.164 +46 + 9 digits, or national 0 + 9 digits, or 9-digit mobile 7[02369]… Rejects common fake / demo numbers; mobile 7[02369] + 7-digit strict run (e.g. 070-123 45 68). */
        isValidSwedishPhone: function (raw) {
            let d = String(raw).replace(/\D/g, "");
            if (d.startsWith("00")) d = d.slice(2);
            if (d.startsWith("46") && d.length === 11) {
                if (!/^46\d{9}$/.test(d)) return false;
            } else if (d.startsWith("0") && d.length === 10) {
                d = "46" + d.slice(1);
                if (!/^46\d{9}$/.test(d)) return false;
            } else if (d.length === 9 && /^7[02369]\d{7}$/.test(d)) {
                d = "46" + d;
                if (!/^46\d{9}$/.test(d)) return false;
            } else {
                return false;
            ...
            var nsn = d.slice(2);
            if (/^(\d)\1{8}$/.test(nsn)) return false;
            var fakeSeq = ["012345678", "123456789", "987654321", "876543210"];
            for (var si = 0; si < fakeSeq.length; si++) if (nsn === fakeSeq[si]) return false;
            var fakeE164 = { "46720000000": 1, "46700000000": 1, "46701234567": 1 };
            if (fakeE164[d]) return false;
            if (/^70{8}$/.test(nsn)) return false;
            if (/0{7}/.test(nsn)) return false;
            if (/^70123456\d$/.test(nsn)) return false;
            if (/^01234567\d$/.test(nsn)) return false;
            if (/^12345678\d$/.test(nsn)) return false;
            if (/^98765432\d$/.test(nsn)) return false;
            if (/^87654321\d$/.test(nsn)) return false;
            if (/^7[02369]/.test(nsn)) {
                var rest = nsn.slice(2);
                if (rest.length === 7) {
                    var ascM = true,
                        descM = true;
                    for (var ri = 0; ri < 6; ri++) {
                        var ra = rest.charCodeAt(ri) - 48,
                            rb = rest.charCodeAt(ri + 1) - 48;
                        if (rb !== ra + 1) ascM = false;
                        if (rb !== ra - 1) descM = false;
                    }
                    if (ascM || descM) return false;
                }
            }
            return true;
        },
        ...
        getReportWorkerBase: function () {
            const w = typeof window !== "undefined" ? window.selmaReportWorkerBase : undefined;
            const c = this.config.reportWorkerBaseUrl || "";
            if (typeof w === "string") {
        ...
        /** For POST /internal/report-ready only — set window.selmaReportWorkerInternalSecret (never commit real prod secrets). */
        getReportWorkerInternalSecret: function () {
            const w = typeof window !== "undefined" ? window.selmaReportWorkerInternalSecret : undefined;
            if (typeof w === "string" && w.trim()) return w.trim();
            return (this.config.reportWorkerInternalSecret || "").trim();
        },

        postToReportWorkerScan: function () {
            const base = this.getReportWorkerBase();
            if (!base) {
                console.log("[Report worker] skip POST /api/scan (no base URL)");
                return;
            }
            const website = "https://" + this.session.domain;
            console.log("[Report worker] POST /api/scan →", base + "/api/scan", website);
            const self = this;
            fetch(base + "/api/scan", {
                method: "POST",
                ...
                .then(function (data) {
                    if (data && data.jobId) self.session.workerJobId = data.jobId;
                    console.log("[Report worker] POST /api/scan", data);
                })
                .catch(function (e) {
                    console.warn("[Report worker] POST /api/scan failed", e);
                });
        },

        postToReportWorkerContact: function () {
            const base = this.getReportWorkerBase();
            if (!base || !this.session.workerJobId) {
                console.log("[Report worker] skip POST /api/contact", {
                    hasBase: !!base,
                    workerJobId: this.session.workerJobId,
                });
                return;
            }
            console.log("[Report worker] POST /api/contact →", base + "/api/contact", this.session.workerJobId);
            fetch(base + "/api/contact", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    jobId: this.session.workerJobId,
                ...
                .then(function (data) {
                    console.log("[Report worker] POST /api/contact", data);
                })
                .catch(function (e) {
                    console.warn("[Report worker] POST /api/contact failed", e);
                });
        },

        /** POST /internal/report-ready (Bearer) when ACE results include reportId / report URL. */
        postToReportWorkerReportReady: function () {
            const base = this.getReportWorkerBase();
            const secret = this.getReportWorkerInternalSecret();
            if (!base || !this.session.workerJobId) {
                console.log("[Report worker] skip POST /internal/report-ready", {
                    hasBase: !!base,
                    workerJobId: this.session.workerJobId,
                });
                return;
            ...
            if (!secret) {
                console.log("[Report worker] skip POST /internal/report-ready (no internal secret — set window.selmaReportWorkerInternalSecret)");
                return;
            }
            const reportId = this.session.reportId;
            const aceReportUrl =
                this.session.brandedReportUrl ||
                this.session.whitelabelReportUrl ||
                null;
            if (!reportId && !aceReportUrl) {
                console.log("[Report worker] skip POST /internal/report-ready (no reportId / aceReportUrl yet)");
                return;
            }
            const payload = { jobId: this.session.workerJobId };
            if (reportId) payload.reportId = reportId;
            if (aceReportUrl) payload.aceReportUrl = aceReportUrl;
            const url = base + "/internal/report-ready";
            console.log("[Report worker] POST /internal/report-ready →", url, {
                jobId: payload.jobId,
                reportId: payload.reportId || null,
                hasAceReportUrl: !!payload.aceReportUrl,
            });
            fetch(url, {
                method: "POST",
                headers: {
                ...
                .then(function (data) {
                    console.log("[Report worker] POST /internal/report-ready", data);
                })
                .catch(function (e) {
                    console.warn("[Report worker] POST /internal/report-ready failed", e);
                });
        },
        ...
        processAccessiBeResults: function (data) {
            try {
                ...
                */
                ...
                this.postToReportWorkerReportReady();
